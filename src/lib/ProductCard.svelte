...
<div class=" rounded p-2 space-y-2 hidden grid justify-items-center bg-slate-50 text-center  place-items-end ">
	<img
		class="bg-slate-200 h-auto w-auto object-contain"
		src={'$lib/bedroom/'+imgurl+".jpg"}}
		alt={item.name}
	/>
	<a href={pathname}/details/{item.id} class="text-center text-gray-600 "
		>{item.name}</a
	>
...
<div class="relative m-x-10 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
  <a class="relative mx-3 mt-3 flex  w-full overflow-hidden rounded-xl" href="#">
  ...
  <div class="mt-4 px-5 pb-5">
    ...
    <div class="mt-2 mb-5 flex items-center justify-between">
      <p>
        <span class="text-3xl font-bold text-slate-900">{item.special_price}</span>
