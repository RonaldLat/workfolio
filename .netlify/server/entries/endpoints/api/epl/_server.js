...
async function getEpl(url, season) {
  ...
  table.find("tbody tr").each((i, row) => {
    ...
    rowData.average_points = $(row).find("[data-stat=points_avg]").text().trim();
    rowData.goals_for = $(row).find("[data-stat=goals_for]").text().trim();
    rowData.goals_against = $(row).find("[data-stat=goals_against]").text().trim();
    ...
    rowData.xg_diff_per90 = $(row).find("[data-stat=xg_diff_per90]").text().trim();
    rowData.attendance = $(row).find("[data-stat=attendance_per_g]").text().trim();
    rowData.top_keeper = $(row).find("[data-stat=top_keeper]").text().trim();
    console.log(rowData);
    const last5Games = [];
    $(row).find("[data-stat=last_5] .poptip").each((i2, game) => {
      const gameData = {};
      gameData.result = $(game).find("a").text().trim();
      gameData.score = $(game).attr("tip").replace(/.*<br>/, "").trim();
      gameData.date = $(game).attr("tip").replace(/<br>.*/, "").replace(/<strong>|<\/strong>/g, "").trim();
      last5Games.push(gameData);
    });
    rowData.last_5_games = last5Games;
    const topScorersData = {};
    $(row).find("[data-stat=top_team_scorers] ").each((i2, player) => {
      const players = [];
      let topScorerTag = $(player).find("a");
      topScorersData.goals = $(player).find("span").text();
      if (topScorerTag.length > 1) {
        topScorerTag.each((i3, j) => {
          players.push($(j).text());
...
async function GET() {
  const url = "https://fbref.com/en/comps/9/2022-2023/2022-2023-Premier-League-Stats";
