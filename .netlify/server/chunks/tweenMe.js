  ...
  _addToTimeline = function _addToTimeline2(
    ...
    child._start = _roundPrecise(
      (_isNumber$1(position)
        ...
        ? _parsePosition$1(timeline2, position, child)
  ...
  _createPlugin = function _createPlugin2(config3) {
    if (_windowExists$2() && config3) {
      ...
      if (config3 !== Plugin) {
        ...
        _setDefaults$1(
          Plugin,
          _setDefaults$1(_copyExcluding(config3, instanceDefaults), statics)
        );
        _merge(
          Plugin.prototype,
...
var MatchMedia = /* @__PURE__ */ (function () {
  ...
  _proto6.add = function add(conditions, func, scope) {
    ...
    _context$2 &&
      !context3.selector &&
      (context3.selector = _context$2.selector);
...
var gsap$1,
  ...
  _getScrollFunc = function _getScrollFunc2(element, _ref) {
    ...
    prev ||
      (func.smooth =
        gsap$1.getProperty(element, 'scrollBehavior') === 'smooth');
    return func;
  },
...
var Observer = /* @__PURE__ */ (function () {
  ...
  _proto.init = function init4(vars) {
    ...
    lineHeight ||
      (lineHeight =
        parseFloat(_win$1.getComputedStyle(_body$1).lineHeight) || 22);
    var id,
      onStopDelayedCall,
      dragged,
      moved,
    ...
    self.enable = function (e) {
      if (!self.isEnabled) {
        ...
        type.indexOf('wheel') >= 0 &&
          _addListener$1(target, 'wheel', _onWheel, preventDefault, capture);
        if (
          ...
          allowClicks &&
            _addListener$1(target, 'click', clickCapture, false, true);
...
var gsap,
  ...
  _keepClamp = function _keepClamp2(value, clamp3) {
    return clamp3 && (!_isString2(value) || value.substr(0, 6) !== 'clamp(')
      ? 'clamp(' + value + ')'
      : value;
  },
  ...
  _getGSAP2 = function _getGSAP3() {
    return (
      gsap ||
      (_windowExists3() && (gsap = window.gsap) && gsap.registerPlugin && gsap)
    );
  },
  ...
  _addListener2 = function _addListener3(
    element,
    type,
    func,
    nonPassive,
  ...
  _softRefresh = function _softRefresh2() {
    return (
      _removeListener2(ScrollTrigger, 'scrollEnd', _softRefresh2) ||
      _refreshAll(true)
    );
  },
  ...
  _getTweenCreator = function _getTweenCreator2(scroller, direction) {
    ...
    ScrollTrigger.isTouch &&
      _addListener2(scroller, 'touchmove', getScroll.wheelHandler);
    return getTween;
  };
var ScrollTrigger = /* @__PURE__ */ (function () {
  ...
  _proto.init = function init4(vars, animation) {
          ...
          if (
            ...
            var progress = (scroll - start) / change,
              ...
              velocity = refreshedRecently
                ? 0
                : ((totalProgress - snap22) / (_getTime() - _time2)) * 1e3 || 0,
    ...
    if (markers) {
      markerVars = _isObject2(markers)
        ? _setDefaults2(markers, _markerDefaults)
        : _markerDefaults;
    ...
    self.refresh = function (soft, force, position, pinOffset) {
      ...
      if (_isString2(parsedEnd) && !parsedEnd.indexOf('+=')) {
        if (~parsedEnd.indexOf(' ')) {
          parsedEnd =
            (_isString2(parsedStart) ? parsedStart.split(' ')[0] : '') +
            parsedEnd;
        } else {
          ...
          if (toggled || !_limitCallbacks) {
            ...
            if (!toggled) {
              ...
              callbacks[toggleState] &&
                _callback2(self, callbacks[toggleState]);
            }
          }
          if (
            fastScrollEnd &&
            ...
            Math.abs(self.getVelocity()) >
              (_isNumber2(fastScrollEnd) ? fastScrollEnd : 2500)
          ) {
            _endAnimation(self.callbackAnimation);
            scrubTween
              ? scrubTween.progress(1)
    ...
    self.enable = function (reset, refresh) {
      if (!self.enabled) {
        ...
        onRefreshInit &&
          _addListener2(ScrollTrigger2, 'refreshInit', onRefreshInit);
        if (reset !== false) {
    ...
    self.disable = function (reset, allowAnimation) {
      if (self.enabled) {
        ...
        onRefreshInit &&
          _removeListener2(ScrollTrigger2, 'refreshInit', onRefreshInit);
        if (snapDelayedCall) {
  ...
  ScrollTrigger2.config = function config3(vars) {
    ...
    if ('autoRefreshEvents' in vars) {
      _iterateAutoRefresh(_removeListener2) ||
        _iterateAutoRefresh(_addListener2, vars.autoRefreshEvents || 'none');
  ...
  ScrollTrigger2.isInViewport = function isInViewport(
    ...
    var bounds = (
        _isString2(element) ? _getTarget(element) : element
...
ScrollTrigger.batch = function (targets, vars) {
  ...
  for (p in vars) {
    varsCopy[p] =
      p.substr(0, 2) === 'on' && _isFunction2(vars[p]) && p !== 'onRefreshInit'
  ...
  _getScrollNormalizer = function _getScrollNormalizer2(vars) {
    ...
    var _vars2 = vars,
      ...
      removeContentOffset = function removeContentOffset2() {
        content._gsap.y =
          _round2(parseFloat(content._gsap.y) + scrollFuncY.offset) + 'px';
